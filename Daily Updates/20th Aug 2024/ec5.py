...
def generate_requirements(skill, job_description):
    prompt = f"Briefly describe how much the skill '{skill}' is required for the job role in the following job description.\n\nJob Description:\n{job_description}"
...
# Remove the second row (index 1) and the last row
evaluation_sheet = evaluation_sheet.drop([1, evaluation_sheet.index[-1]])


# Define the file path for saving the evaluation sheet
output_file = "evaluation_sheet_first_record_updated5.xlsx"
